...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/12/07 14:09:26 by ccalas            #+#    #+#             */
/*   Updated: 2021/12/07 14:09:28 by ccalas           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	char		*line;
	static char	*line_wip[1024];

	if (fd < 0 || BUFFER_SIZE <= 0)
		return (NULL);
	line_wip[fd] = ft_read_fd(fd, line_wip[fd]);
	if (!line_wip[fd])
		return (NULL);
	line = ft_line(line_wip[fd]);
	line_wip[fd] = ft_new_start(line_wip[fd]);
	return (line);
}
